  ...
  return (
    <div className={`${styles[card]}`}>
      <div className={styles["info-card-top"]}>
        ...
        <Button text={action} onClick={onAddDriverClick}></Button>
      </div>
      <div className={styles["info-card-bottom"]}>
        <p className={styles.description}>
          {cardType.charAt(0).toUpperCase() + cardType.slice(1)}
        </p>
        {cardType === "balance" && <h2 className={styles.amount}>R{amount}</h2>}
        {cardType !== "balance" && <h2 className={styles.amount}>{amount}</h2>}
      </div>
    </div>
  );
}
